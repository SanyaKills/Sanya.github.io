...
scene.add(light);
scene.add(light2);
const occluder = await loadGLTF("Gltf/headOccluder.glb");
...
const glasses = await loadGLTF("Gltf/glasses/scene.gltf");
glasses.scene.rotation.set(0, -Math.PI/2, 0);
glasses.scene.position.set(0, -0.45, 0);
glasses.scene.scale.set(0.085, 0.085, 0.085);
glasses.scene.renderOrder = 1;
const glassesAnchor = mindarThree.addAnchor(168);
glassesAnchor.group.add(glasses.scene);

const glasses2 = await loadGLTF("Gltf/glasses2/scene.gltf");
glasses2.scene.rotation.set(0, 0, 0);
glasses2.scene.position.set(0, -0.02, 0);
glasses2.scene.scale.set(0.002, 0.002, 0.002);
glasses2.scene.renderOrder = 1;
const glasses2Anchor = mindarThree.addAnchor(168);
glasses2Anchor.group.add(glasses2.scene);

const hat1 = await loadGLTF("Gltf/hat/scene.gltf");
hat1.scene.position.set(0, -0.1, -0.5);
hat1.scene.scale.set(0.7, 0.7, 0.7);
hat1.scene.renderOrder = 1;
const hat1Anchor = mindarThree.addAnchor(10);
hat1Anchor.group.add(hat1.scene);

const hat2 = await loadGLTF("Gltf/hat2/scene.gltf");
hat2.scene.position.set(0, 0.05, -0.5);
hat2.scene.scale.set(1, 1, 1);
hat2.scene.renderOrder = 1;
const hat2Anchor = mindarThree.addAnchor(10);
hat2Anchor.group.add(hat2.scene);

const buttons =
["#glasses1", "#glasses2", "#hat1", "#hat2",];
